...
src/contracts/           Pydantic config, TrialRequest, TrialResult, SandboxSession, ScoreResult
src/experiments/
...
Expected: compiler + verifier + OpenHands adapter tests pass (no Docker, no HF key, no OpenHands binary).
...
- **Hugging Face**: needs `HF_API_KEY` in `.env`. There is no end-to-end trial runner yet, so a live generate call is manual.
- **Real Docker verifier**: `test_verifier.py` does not start containers. A live compose run needs Docker Desktop and is still unwired to a trial runner.
...
## Not built yet
Trial runner, Harbor orchestration, reporting.
